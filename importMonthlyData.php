...
try {
    ...
    foreach ($input as $index => $row) {
        ...
        if ($checkStmt->fetchColumn() > 0) {
            $errors[] = "第".($index+1)."行：身份证号 {$idNumber} 在 {$yearMonth} 月份已存在记录，不能重复导入";
            continue;
        }
        ...
        try {
            ...
            if ($e->getCode() == 23000 && strpos($e->getMessage(), 'Duplicate entry') !== false) {
                ...
                // 其他数据库错误
                $errors[] = "第".($index+1)."行：数据库错误 - " . $e->getMessage();
            }
            continue;
        }
    } // foreach 结束
